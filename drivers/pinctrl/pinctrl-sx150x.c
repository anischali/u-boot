 ...
 * inspired and adapted from linux driver of sx150x written by Gregory Bean <gbean@codeaurora.org>
 */
...
static const struct sx150x_pin_desc sx150x_16_pins[] = {
	...
	SX150X_PIN(8, "gpio8"),
	SX150X_PIN(9, "gpio9"),
	SX150X_PIN(10, "gpio10"),
	SX150X_PIN(11, "gpio11"),
	SX150X_PIN(12, "gpio12"),
	SX150X_PIN(13, "gpio13"),
	SX150X_PIN(14, "gpio14"),
	SX150X_PIN(15, "gpio15"),
	SX150X_PIN(16, "oscio"),
};
...
static bool sx150x_pin_is_oscio(struct sx150x_pinctrl_priv *pctl, unsigned int pin)
{
...
static int sx150x_reg_width(struct sx150x_pinctrl_priv *pctl,
			    unsigned int reg)
{
...
static int sx150x_write_bits(struct sx150x_pinctrl_priv *pctl, uint reg, uint mask, uint val)
{
	int orig, tmp;
...
static int sx150x_gpio_set(struct  udevice *dev, unsigned int offset,
			   int value)
{
	struct sx150x_pinctrl_priv *pctl = dev_get_priv(dev->parent);

	return sx150x_write_bits(pctl, pctl->data->reg_data, BIT(offset), value ? BIT(offset) : 0);
}

static int sx150x_gpio_oscio_set(struct udevice *dev,
				 int value)
{
	struct sx150x_pinctrl_priv *pctl = dev_get_priv(dev->parent);

	return sx150x_write(pctl, pctl->data->pri.x789.reg_clock, (value ? 0x1f : 0x10));
}
...
static int sx150x_gpio_direction_input(struct udevice *dev, unsigned int offset)
{
	...
	return sx150x_write_bits(pctl, pctl->data->reg_dir, BIT(offset), BIT(offset));
}

static int sx150x_gpio_direction_output(struct udevice *dev, unsigned int offset,
					int value)
{
	struct sx150x_pinctrl_priv *pctl = dev_get_priv(dev->parent);
	int ret;
...
static const struct udevice_id sx150x_pinctrl_of_match[] = {
	{ .compatible = "semtech,sx1501q", .data = (ulong)&sx1501q_device_data },
	{ .compatible = "semtech,sx1502q", .data = (ulong)&sx1502q_device_data },
	{ .compatible = "semtech,sx1503q", .data = (ulong)&sx1503q_device_data },
	{ .compatible = "semtech,sx1504q", .data = (ulong)&sx1504q_device_data },
	{ .compatible = "semtech,sx1505q", .data = (ulong)&sx1505q_device_data },
	{ .compatible = "semtech,sx1506q", .data = (ulong)&sx1506q_device_data },
	{ .compatible = "semtech,sx1507q", .data = (ulong)&sx1507q_device_data },
	{ .compatible = "semtech,sx1508q", .data = (ulong)&sx1508q_device_data },
	{ .compatible = "semtech,sx1509q", .data = (ulong)&sx1509q_device_data },
	{},
};
...
static const char *sx150x_pinctrl_get_pin_name(struct udevice *dev, unsigned int selector)
{
	struct sx150x_pinctrl_priv *pctl = dev_get_priv(dev);
				   ...
				   unsigned int arg)
{
	...
	case PIN_CONFIG_BIAS_DISABLE:
		ret = sx150x_write_bits(pctl, pctl->data->reg_pulldn, BIT(pin), 0);
		if (ret < 0)
			return ret;

		ret = sx150x_write_bits(pctl, pctl->data->reg_pullup, BIT(pin), 0);
		if (ret < 0)
			return ret;
		break;

	case PIN_CONFIG_BIAS_PULL_UP:
		ret = sx150x_write_bits(pctl, pctl->data->reg_pullup, BIT(pin), BIT(pin));
		if (ret < 0)
			return ret;

		break;
	case PIN_CONFIG_BIAS_PULL_DOWN:
		ret = sx150x_write_bits(pctl, pctl->data->reg_pulldn, BIT(pin), BIT(pin));
		if (ret < 0)
			return ret;
		break;
	...
	case PIN_CONFIG_DRIVE_OPEN_DRAIN:
		...
		ret = sx150x_write_bits(pctl, pctl->data->pri.x789.reg_drain, BIT(pin), BIT(pin));
		if (ret < 0)
			return ret;
	...
	case PIN_CONFIG_DRIVE_PUSH_PULL:
		...
		ret = sx150x_write_bits(pctl, pctl->data->pri.x789.reg_drain, BIT(pin), 0);
		if (ret < 0)
			return ret;
...
static int sx150x_pinctrl_probe(struct udevice *dev)
{
	...
	if (ret) {
		err("Cannot initialize GPIO expander at %02x with %d", reg, ret);
		return ret;
	}
